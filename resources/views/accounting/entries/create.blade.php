...
@section('page-title', 'Nuevo Comprobante Contable')
@section('breadcrumb')
    <a href="{{ route('contable.entries.index') }}">Comprobantes</a>
    <span class="mx-1 text-ink/30">/</span> Nuevo
@endsection
...
<div x-data="entryForm(
    ...
    {{ Js::from($costCenters) }},
    [],
    '{{ $entryTypes->first()?->code ?? '' }}'
)" class="space-y-4">

<form id="entry-form" method="POST" action="{{ route('contable.entries.store') }}">
@csrf
    ...
    <div class="p-5">
        <div class="grid grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
            ...
            <div>
                ...
                <select name="entry_type" x-model="entryType" @change="updateSequences()" required
            ...
            <div>
                <label class="block text-[10px] font-bold text-ink/50 uppercase tracking-wider mb-1.5">
                    Numeración <span class="text-red-400">*</span>
                </label>
                <select id="seq-select" name="entry_sequence_id"
                    class="w-full border border-ink/15 px-3 py-2 text-[12px] text-ink bg-white
                           focus:outline-none focus:border-sage">
                </select>
            </div>
                    ...
                    <tr class="border-b border-ink/8 hover:bg-cream/60 transition-colors">
                        ...
                        <td class="px-0 py-0 border-r border-ink/8 min-w-56">
                            <div x-data="accountCombo(line)"
                                ...
                                {{-- Dropdown (position:fixed para escapar overflow) --}}
                                <div x-show="open"
                                     x-cloak
                                     :style="dropStyle"
...
@push('scripts')
<script>
const SEQUENCE_MAP = @json($entryTypes->keyBy('code')->map(fn($t) => $t->sequences->values()));

function fillSequenceSelect(typeCode) {
    const sel  = document.getElementById('seq-select');
    if (!sel) return;
    const seqs = SEQUENCE_MAP[typeCode] ?? [];
    sel.innerHTML = seqs.length
        ? seqs.map(s => `<option value="${s.id}">${s.name}</option>`).join('')
        : '<option value="">Sin numeración</option>';
}

function entryForm(accounts, costCenters, existingLines, entryType) {
    return {
        accounts,
        costCenters,
        entryType: entryType || '',
        ...
        init() {
            fillSequenceSelect(this.entryType);
        ...
        updateSequences() { fillSequenceSelect(this.entryType); },
